			...
			if($arManuf = $rsManuf->GetNext()){
				...
				if($arModel = $rsModel->GetNext()){
					...
					if($arType = $rsType->GetNext()){
						...
						$Node=10001;
						$arResult['SECTIONS'] = GetSectionsByNode($Node,$Brand,$MOD_ID,$TYP_ID,TECDOC_LNG_ID);
...
function GetSectionsByNode($Node,$Brand,$MOD_ID,$TYP_ID,$LNG_ID){
	$rsSec = TDSQL::GetSections($TYP_ID,$Node,$LNG_ID);
	while($arSec = $rsSec->GetNext()){
		$arSec['NAME'] = TDSecGetName($arSec['STR_DES_TEXT']);
		if($arSec['DESCENDANTS']>0){
			$arSec['ICON'] = 'folder';
			$arSec['STATE'] = 'closed';
			$arSec['HREF'] = '#';
			$arSec['SECTIONS'] = GetSectionsByNode($arSec['STR_ID'],$Brand,$MOD_ID,$TYP_ID,$LNG_ID);
		}else{
			$arSec['ICON'] = 'file';
			$arSec['HREF'] = CORE_ROOT_DIR.'/'.$Brand.'/m'.$MOD_ID.'/t'.$TYP_ID.'/s'.$arSec['STR_ID'].'/"';
		}
		$SECTIONS[] = $arSec;
	}
	return $SECTIONS;
